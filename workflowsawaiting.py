...
def main():
    ...
    repositories_report = []
    repositories = get_repositories(configuration_list=configuration_list)
    for repository in repositories:
        verbose_print("Getting workflow runs for", repository["object"].full_name)
        verbose_print("permissions:", repository["permissions"])
        repository_report = {
            "name": repository["object"].full_name,
            "permissions": repository["permissions"],
            "runs": get_runs(repository=repository),
        }
...
def get_repositories(configuration_list):
    """Returns the list of repositories defined by the configuration
    ...
    for configuration_element in configuration_list:
        ...
        if repository_name:
            repository = {"object": gh_api.repos.get(owner=owner_name, repo=repository_name)}
            repository["permissions"] = get_permissions(repository_object=repository["object"])
            if in_scope(repository=repository, configuration_element=configuration_element):
...
def in_scope(repository, configuration_element):
    """Returns whether the given repository is in scope for the configuration
    ...
    return not repository["object"].archived and (
        repository["permissions"] == "write" or repository["permissions"] == "admin"
    )


def get_runs(repository):
    """Returns a list of URLs for workflow runs awaiting approval in the given repo
    ...
    runs_pages = ghapi.page.paged(
        ...
        owner=repository["object"].owner.login,
        repo=repository["object"].name,
        event="pull_request",
